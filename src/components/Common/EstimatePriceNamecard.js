...
function getCoatingFee(coatingType = '없음', totalQty = 500, perSheetCount = 1) {
  ...
  // 전지 1장당 실제 생산수(perSheetCount)로 sheetCount 계산
  const sheetCount = Math.ceil(totalQty / perSheetCount);

  let totalCoatingFee = 0;
  if (coatingType === '벨벳') {
    totalCoatingFee = (sheetCount <= 2) ? 200000 : sheetCount * 400;
  } else {
    totalCoatingFee = (sheetCount <= 2) ? 100000 : sheetCount * 200;
  }

  // **perSheetCount로 1장 단가 환산**
  const perUnitCoatingFee = totalCoatingFee / perSheetCount;
  return Math.ceil(perUnitCoatingFee * totalQty);
}
